...
Using the `disableMerge` built-in Reviewpad will fail the check `reviewpad merge gate` and close the merge gate.
...
GitHub repository and add the [status check](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/collaborating-on-repositories-with-code-quality-features/about-status-checks) `reviewpad merge gate` to the list of required status checks.
